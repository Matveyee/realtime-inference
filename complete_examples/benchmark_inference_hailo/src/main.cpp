    ...
    while (!eos) {
       ...
       // log("put packet");
        ...
        //    std::cout << "In decode " << frame << std::endl;
            ...
            start = simple_now();
            std::cout <<"\r delay:" <<  cap_index - hailo.frames_processed << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    mpp.init();
    hailo.init(result["model-path"].as<std::string>());
    ...
    double sum = 0;
    for (int i = 0; i < hailo.times.size(); i++) {

        sum += hailo.times[i];

    }
    
    std::cout << "Average Inference FPS = " << hailo.times.size() / (sum / 1000) << std::endl;
