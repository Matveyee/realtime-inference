...
void HailoNN::inference(standart_inference_ctx* ctx) {
    ...
    auto job = infer_model1.run_async(bindings,[&, start](const hailort::AsyncInferCompletionInfo & info){
...
void HailoNN::inference_dmabuf(standart_inference_ctx* ctx) {
    ...
    auto job = infer_model1.run_async(bindings,[&, start](const hailort::AsyncInferCompletionInfo & info){
        ...
        // Здесь можно получить результат работы нейроести через output_buffers
