...
void RockChipNN::inference_dmabuf(standart_inference_ctx* app_ctx)
{
    int ret;

    // 1. Убедимся, что у нас есть виртуальный адрес dma-buf
    if (app_ctx->input_buffer == nullptr) {
        void* addr = mmap(nullptr,
                          ...
                          0);
        if (addr == MAP_FAILED) {
            perror("mmap dma_fd failed");
            return;
        }

        app_ctx->input_buffer = static_cast<uint8_t*>(addr);
    }

    // Если буфер заполняет не CPU, а железо (камера/RGA),
    // и ты потом читаешь/меняешь его на CPU — тут можно вставить
    // cache flush / msync при необходимости.
    // msync(app_ctx->input_buffer, app_ctx->dma_size, MS_SYNC);

    // 2. Готовим rknn_input точь-в-точь как раньше
    rknn_input input;
    memset(&input, 0, sizeof(input));

    input.index        = 0;
    input.pass_through = 0;                        // пусть RKNN сам делает препроцесс
    input.type         = RKNN_TENSOR_UINT8;        // как было у тебя
    input.fmt          = RKNN_TENSOR_NHWC;         // NHWC, как раньше
    input.buf          = app_ctx->input_buffer;    // теперь это mmap dma-buf
    input.size         = 640 * 640 * 3;            // твой RGB888 без паддинга

    ret = rknn_inputs_set(ctx, io_num.n_input, &input);
    if (ret < 0) {
        printf("rknn_inputs_set error %d\n", ret);
        return;
    }

    // 3. Запуск инференса (как раньше)
    ...
    if (ret < 0) {
        printf("rknn_outputs_get error %d\n", ret);
        free(outputs);
        return;
    }
